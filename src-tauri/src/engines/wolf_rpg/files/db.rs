use crate::models::translation::{TextUnit, PromptType, TranslationStatus};
use crate::engines::wolf_rpg::files::regex::{is_translatable_wolf_text, wolf_replace_placeholders_for_translation, wolf_restore_placeholders_after_translation};
        ...
        for (type_idx, type_obj) in types.iter().enumerate() {
            if let Some(data_array) = type_obj.get("data").and_then(|v| v.as_array()) {
                for (data_idx, data_obj) in data_array.iter().enumerate() {
                    extract_from_db_data_entry(&mut text_units, data_obj, file_path, type_idx, data_idx);
                }
            }
        }
    }
...
fn is_translatable_db_value(value: &str) -> bool {
    ...
    let file_extensions = Regex::new(r"\.(png|jpg|jpeg|gif|bmp|wav|mp3|ogg|txt|json|dat)$").unwrap();
