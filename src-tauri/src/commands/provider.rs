...
pub async fn test_llm_connection(config: LlmConfig) -> Result<bool, String> {
    log::debug!("Testing LLM connection for provider: '{}', model: '{}'", 
...
struct ModelCatalog { models: Vec<ModelInfo> }
...
pub fn get_models(provider: String) -> Result<Vec<ModelInfo>, String> {
    ...
    let json = match key.as_str() {
        ...
        "openrouter" => include_str!("../../models/openrouter.json"),
        "groq" => include_str!("../../models/groq.json"),
    ...
    log::debug!("Returning {} models for provider '{}'", parsed.models.len(), provider);
    Ok(parsed.models)
}
